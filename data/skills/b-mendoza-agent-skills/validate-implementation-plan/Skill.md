...
answer summaries — not raw plan text.
...
2. `NormalizeInputs` → `AuthorizeArtifacts` (ask before overwrite) →
   `EstablishOrigin` (one baseline question if inadequate) → `ClassifyContext`.
3. `DispatchSnapshot` → `DispatchRequirements` → optional `DispatchEvidence`
   (or `RecordEvidenceGap` when core audit remains viable).
4. `DispatchAuditors` (three discovery auditors). On failure, `RetryAuditor`
   re-dispatches only the failed branch into `DispatchAuditors` (≤3 cycles).
5. If decision-relevant unresolved assumptions: `AskAssumptions` →
   `ResolveAssumptions` → `GateOpenQuestions`.
